...
Я решил создать эту библиотеку потому что часто пользовался `react-transition-group`, но мне не нравились его некоторые моменты. Например, помимо того как прописывать `duration` в css, нужно передавать `timeout` или `addEndListener`. Также там нету "move" анимации и совместимости с React Strict Mode.
...
По дефолту, дочерний элемент будет демонтирован при исчезновения. Но если вы хотите чтобы он был скрыт с помощью `display: none`, можно передать проп `unmount={false}`.
...
| имя | тип | значение по умолчанию | описание |
| --------- | -------------------- | -------------------- | --------------------------------------------------------------------------------------- |
| name      | `string`             | `transition`         | Имя для дочерних `<Transition>` компонентов. Также используется для генерации `moveClass` пропа, если он не передан  |
